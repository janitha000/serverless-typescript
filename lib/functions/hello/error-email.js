...
const cwl = new CloudWatchLogs({ apiVersion: "2014-03-28", region: "ap-southeast-1" });
...
let generateEmailContent = (data, message) => {
    ...
    let text = `Alarm Name:<b>${message.AlarmName}</b><br/>Details: <a href="https://my.example.com">Production URL</a><br/>Account ID:${message.AWSAccountId}<br/>Region:${message.Region}<br/>Alarm Time:${date.toString()}<br/>${logData}`;
    let subject = `Details for Alarm - ${message.AlarmName} [URGENT]`;
    let emailContent = {
        Destination: {
            ToAddresses: ["janitha000@gmail.com"]
        },
        Message: {
            Body: {
                Html: {
...
module.exports.dispatchErrors = (event, context, _cb) => {
...
//# sourceMappingURL=error-email.js.map